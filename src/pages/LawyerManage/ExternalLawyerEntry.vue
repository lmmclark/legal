								...
								<Form ref="baseInfo" :model="baseInfo" :rules="ruleBaseInfo" label-position="right" :label-width="140">
									...
									<Row>
										...
										<FormItem label="执业证号:" prop="occupationCertificateNo">
											<Input class="forminput" v-model="baseInfo.occupationCertificateNo" placeholder="清收执业证号"></Input>
										</FormItem>
										</Col>
										<Col span="10" offset="3">
									...
									<Row>
										...
										<Col span="10">
										<FormItem label="擅长业务范围:" prop="areasExpertiseSecondId" class="business1">
											<Select class="forminput" v-model="baseInfo.areasExpertiseSecondId" placeholder="请选择">
												<Option v-for="item in scajlx" :value="item.id" :key="item.id">{{item.dictName}}</Option>
											</Select>
										</FormItem>
										</Col>
									...
									<Row>
										...
										<FormItem label="相关附件:" prop="attachmentInfoDTOs">
											<Upload name="attachmentFile" :max-size='10240' :before-upload="beforeUpload" :on-progress="upLoadProgress" :on-success="upLoadSuccess" :on-remove="removeUpload" action="http://10.71.203.67:8658/attachmentInfo/attachmentUpload">
